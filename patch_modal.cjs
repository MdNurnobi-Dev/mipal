          ...
          <div className="bg-white text-slate-800 p-3 px-4 flex justify-between items-center border-b border-slate-200 shrink-0 shadow-sm">
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button 
                ...
                className="w-8 h-8 flex items-center justify-center rounded-lg bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors ml-1 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
...
const newStr = `      {/* Task Execution Modal (Video Ad & Direct Website Visit with Countdown) */}
      {activeTask && (activeTask.type === 'Video' || activeTask.type === 'Website') && (
        <div className="fixed inset-0 z-50 bg-white flex flex-col">
          {/* Top Countdown & Status Bar */}
          <div className="bg-slate-900 text-white p-2.5 px-3 flex justify-between items-center shrink-0 shadow-md relative z-10">
            <div className="flex items-center gap-2 min-w-0">
              {activeTask.type === 'Video' ? (
                <div className="w-7 h-7 rounded bg-orange-500/20 text-orange-400 flex items-center justify-center shrink-0">
              ...
              <div className="min-w-0">
                <h3 className="font-bold text-[11px] flex items-center gap-1.5 truncate">
                  <span className="truncate text-slate-100">{activeTask.title}</span>
                  <span className="text-[9px] font-black text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-1.5 py-0.5 rounded shrink-0">
                    +{formatCurrency(activeTask.reward)}
                  </span>
                </h3>
                <p className="text-[9px] text-slate-400 flex items-center gap-1.5 truncate">
                  {timer > 0 ? (
                    <>
                      <span className="w-1 h-1 rounded-full bg-emerald-500 animate-pulse"></span>
                      <span>কাউন্টডাউন চলছে: <b>{timer} সেকেন্ড</b></span>
                    </>
                  ) : (
            ...
            <div className="flex items-center gap-2 shrink-0">
              {timer > 0 ? (
                <div className="bg-white/10 text-white px-2 py-1 rounded-md text-[10px] font-mono font-bold flex items-center gap-1.5 border border-white/5">
                ...
                <button 
                  onClick={claimTaskReward}
                  className="bg-emerald-500 hover:bg-emerald-400 text-white px-2.5 py-1.5 rounded-md text-[10px] font-bold transition-all flex items-center gap-1 shadow-[0_0_10px_rgba(16,185,129,0.3)] animate-pulse cursor-pointer"
                >
              ...
              <button 
                ...
                className="w-7 h-7 flex items-center justify-center rounded-md bg-white/5 text-slate-300 hover:bg-white/10 hover:text-white transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>

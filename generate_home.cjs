...
export default function Home() {
  const { currentUser, plans, giveawayBanners, posts, addPost, togglePostLike, addPostComment, sharePost, siteSettings } = useApp();
        ...
        <div className="p-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
          {topEarners.map(earner => (
            <div key={earner.id} className="flex items-center justify-between p-2.5 bg-slate-50 border border-slate-100 rounded-lg">
              ...
              <span className="text-xs font-bold text-green-600">{currency}{earner.earnings.toFixed(2)}</span>
            </div>
          ))}
        </div>
      </div>
            ...
            <div className="flex justify-between items-center mt-2.5">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">
                  Balance: <span className="text-green-600">{currency}{currentUser.balance.toFixed(2)}</span>
                </span>
              </div>
              <button 
                type="submit"
